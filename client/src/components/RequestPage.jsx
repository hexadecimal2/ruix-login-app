...
const RequestPage = () => {
  ...
  const navigate = useNavigate();
    ...
    const handleClick = () => {
       fetch('http://localhost:5000/request', {credentials : 'include'}).then((response) => response.json()).then((data) =>  {
        
        if (data.message === 'success'){
            navigate('/userinfo', {state : data.userdata});
        } else
        {
            alert('Token expired!');
            navigate('/login');
    ...
    <div className="container">
      ...
      <div className="logo">
        ...
        <h1>Request Data</h1>
        <p>Authentication with cookies test</p>

        <div className="form">
          <button onClick={() => handleClick()}>Request Data</button>
        </div>

        <p className="login-link" onClick={() => {navigate('/login')}}> <br/> Return to Log in page</p>
      </div>
